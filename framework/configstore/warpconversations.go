...
func (s *RDBConfigStore) AppendWarpMessages(ctx context.Context, ownerID, conversationID string, messages []tables.TableWarpMessage) error {
	...
	return s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		// dbForUpdate locks the conversation row for the rest of the transaction,
		// which serialises position allocation per thread. Two concurrent appends
		// otherwise both read the same COUNT and assign the same positions, and the
		// reopened transcript renders out of order with no way to tell which came
		// first. SQLite is single-writer, so the clause is omitted there.
		var existing tables.TableWarpConversation
		if err := dbForUpdate(tx).Where("id = ? AND owner_id = ?", conversationID, ownerID).First(&existing).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (s *RDBConfigStore) PruneWarpConversations(ctx context.Context, ownerID string, keep int) (int64, error) {
	...
	err := s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		// updated_at <= what the select saw: a row bumped in between no longer
		// matches and survives, rather than being deleted on a stale decision.
		result := tx.Where("id IN ? AND owner_id = ? AND updated_at <= ?", ids, ownerID, newest).
			Delete(&tables.TableWarpConversation{})
