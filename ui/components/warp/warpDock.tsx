...
export default function WarpDock({ children }: { children: React.ReactNode }) {
	...
	//
	// The Sheet stays mounted and is driven by `open` rather than being rendered
	// conditionally, for the same reason the desktop wrappers below are always
	// mounted: children must keep their position in the tree.
	if (isMobile) {
		return (
			<>
				{children}
	...
	return (
		...
		// Both wrappers stay mounted whether or not the dock is open, and only the
		// aside is conditional. Swapping the root between a fragment and this div
		// moved children to a different position in the tree, so React remounted
		// the entire workspace on every open and close - losing page state, form
		// input and scroll position each time.
