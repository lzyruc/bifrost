import { encodeTurnError, parseWarpFrame, splitWarpFrames, type WarpEvent } from "@/components/warp/warpStream.utils";
		...
		async (history: WarpTurn[], question: string) => {
			...
			try {
				...
				if (!(caught instanceof DOMException && caught.name === "AbortError")) {
					...
					// Already encoded upstream if it carries a colon; a raw throw from
					// fetch (a network failure) does not, and must not be read as a code.
					terminalError = raw.includes(":") ? raw : encodeTurnError(undefined, raw);
				}
			} finally {
