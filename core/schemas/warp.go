...
func WarpOwnerID(userID string) string {
	...
	// The sentinel belongs to unauthenticated history alone. An authenticated
	// caller whose id happens to equal it would otherwise list, read, append to
	// and delete the shared bucket, so it is namespaced out of the way rather
	// than rejected - rejecting would lock that user out of history entirely for
	// a name they did not choose.
	if trimmed == WarpGlobalOwnerID {
		return "user:" + trimmed
	}
	return trimmed
}
